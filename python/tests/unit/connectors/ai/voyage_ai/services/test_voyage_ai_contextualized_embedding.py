...
@pytest.mark.asyncio
async def test_generate_contextualized_embeddings_flat_input(voyage_ai_unit_test_env):
    """Test that a flat list[str] input is accepted (auto-chunking / query format)."""
    mock_result = MagicMock()
    mock_result.embeddings = [[0.1, 0.2, 0.3]]
    ...
    # Flat list of texts (List[str]) — the second supported input format.
    inputs = ["full document text", "another document"]
    embeddings = await embedding_service.generate_contextualized_embeddings(inputs)

    assert isinstance(embeddings, ndarray)
    embedding_service._aclient.contextualized_embed.assert_called_once()
    _, called_kwargs = embedding_service._aclient.contextualized_embed.call_args
    assert called_kwargs["inputs"] == inputs
