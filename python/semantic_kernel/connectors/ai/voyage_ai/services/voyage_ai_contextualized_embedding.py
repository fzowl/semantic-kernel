...
class VoyageAIContextualizedEmbedding(VoyageAIBase, EmbeddingGeneratorBase):
    """VoyageAI by MongoDB Contextualized Embedding Service.

    Generates embeddings that capture both local chunk details and
    global document-level metadata using the voyage-context-4 model
    ...
    async def generate_contextualized_embeddings(
        ...
        **kwargs: Any,
        ...
        Both input formats supported by the official contextualized_embed API are
        accepted (see https://docs.voyageai.com/docs/contextualized-chunk-embeddings):

        - ``list[list[str]]`` — a list of documents, where each inner list contains
          the pre-chunked (client-side chunked) text of one document.
          Example: ``[["chunk1", "chunk2"], ["doc2_chunk1"]]``
        - ``list[str]`` — a flat list of texts, used for auto-chunking or for queries.
          Example: ``["full document text", "another document"]``

        Args:
            inputs: Either a list of chunk lists (one list per document) or a flat
                   list of texts. See above for the two supported formats.
